...
EMPTY_COLUMN16_STATE_FILE = "empty_column16_state.json"

def load_empty_column16_state():
    if os.path.exists(EMPTY_COLUMN16_STATE_FILE):
        with open(EMPTY_COLUMN16_STATE_FILE, "r", encoding="utf-8") as file:
            return set(json.load(file))
    return set()

def save_empty_column16_state(state):
    with open(EMPTY_COLUMN16_STATE_FILE, "w", encoding="utf-8") as file:
        json.dump(list(state), file, ensure_ascii=False, indent=4)
...
async def check_empty_column16(context: CallbackContext):
    ...
    try:
        ...
        sent_messages = load_empty_column16_state()

        cursor.execute("SELECT column1, column2, column3, column4 FROM second_sheet_data WHERE column16 IS NULL OR TRIM(column16) = ''")
        rows = cursor.fetchall()
        ...
        save_empty_column16_state(sent_messages)

    except Exception as e:
        logging.error(f"Помилка під час перевірки порожніх значень у column16: {e}")
    finally:
        if conn:
            conn.close()

# async def check_delivery_true_trigger(context):
#     conn = None
#     try:
#         conn = sqlite3.connect(DATABASE_FILE)
#         cursor = conn.cursor()

#         # 1. Якщо файл порожній — запам'ятати всі поточні TRUE (без надсилання)
#         cursor.execute("SELECT * FROM rzm_data WHERE columnK IN ('TRUE', 'ИСТИНА')")
#         rows = cursor.fetchall()
#         sent_orders = load_true_sent()
#         current_true_orders = {f"{row[1]}_{row[10]}": str(row[0]) for row in rows}
#         if not sent_orders:
#             save_true_sent(current_true_orders)
#             return  # Перший запуск, нічого не надсилаємо

#         updated_orders = dict(sent_orders)

#         # --- Отримуємо telegram_id для username "Закупівля" ---
#         user_conn = sqlite3.connect(DATABASE_FILE)
#         user_cursor = user_conn.cursor()
#         user_cursor.execute("SELECT telegram_id FROM database_app_userdatatelegram WHERE username = ?", ("Закупівля",))
#         user_row = user_cursor.fetchone()
#         zakupivlya_id = user_row[0] if user_row else None
#         user_cursor.close()
#         user_conn.close()
...
#             name = row[2]
#             article = row[3]
#             purchase_code = str(row[10])
#             submit_datetime = row[14]
#             unique_key = f"{order_number}_{purchase_code}"

#             if unique_key in sent_orders:
#                 continue  # вже надсилали

#             msg = (
#                 f"✅ <b>Закупку можна доставляти!</b> ✅\n"
#                 f"🆔 <b>Номер замовлення:</b> {order_number}\n"
#                 f"📦 <b>Назва:</b> {name}\n"
#                 f"🔢 <b>Артикул:</b> {article}\n"
#                 f"🛒 <b>Код закупки:</b> {purchase_code}\n"
#                 f"📅 <b>Дата та час подачі заявки:</b> {submit_datetime}\n"
#             )
#             # Надсилаємо у групу
#             await context.bot.send_message(
#                 chat_id=-1002739662152,
#                 text=msg,
#                 parse_mode="HTML",
#                 message_thread_id=242
#             )
#             # Надсилаємо особисто користувачу "Закупівля"
#             if zakupivlya_id:
#                 await context.bot.send_message(
#                     chat_id=zakupivlya_id,
#                     text=msg,
#                     parse_mode="HTML"
#                 )

#             updated_orders[unique_key] = str(row[0])  # додаємо новий TRUE у словник

#         save_true_sent(updated_orders)  # оновлюємо файл після надсилання

#     except Exception as e:
#         logging.error(f"Помилка в check_delivery_true_trigger: {e}")
#     finally:
#         if conn:
#             conn.close()
...
def draw_header(canvas, doc):
    ...
    canvas.drawCentredString(width / 2, height - 40, "ЗАКУПКИ, ЯКІ МОЖНА ДОСТАВЛЯТИ")
    canvas.setFont('DejaVu', 10)
    canvas.drawCentredString(width / 2, height - 60, f"Дата та час надходження: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}")
    canvas.restoreState()
...
async def check_delivery_true_trigger(context):
    ...
    try:
        ...
        for row in rows:
            ...
            if first_run or unique_key not in sent_orders:
                updated_orders[unique_key] = {
                    "row_id": str(row[0]),
        ...
        if new_orders:
            ...
            # Таблиця з перенесенням тексту
            table_data = [
                [
                    Paragraph("Номер замовлення", styles['Normal']),
                    Paragraph("Назва", styles['Normal']),
                    Paragraph("Артикул", styles['Normal']),
                    Paragraph("Код закупки", styles['Normal']),
                    Paragraph("Дата та час подачі заявки", styles['Normal'])
                ]
            ]
            ...
            table = Table(table_data, colWidths=[120, 260, 120, 160, 180], repeatRows=1)
            ...
            doc.build([table], onFirstPage=draw_header, onLaterPages=draw_header)
            pdf_buffer.seek(0)

            await context.bot.send_document(
                chat_id=-1002739662152,
                document=pdf_buffer,
                filename=f"zakupki_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                caption="✅ Закупки, які можна доставляти!",
                message_thread_id=242
            )

            pdf_buffer.seek(0)  # Повертаємося на початок для другого надсилання
