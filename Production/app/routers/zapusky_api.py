...
def _extract_details(pdf_text: str):
    ...
    # Шукаємо всі збіги по ВСЬОМУ тексту PDF (ігноруючи розриви рядків).
    # (?P<ім'я>) - це іменовані групи для зручності.
    # Зверніть увагу: у групі 'code' немає пробілу! Це відсіче сміття типу "4 * 7 4 x".
    pattern = re.compile(
        r"(?P<num>\d+)\s+"                                     # 1. Номер деталі
        r"(?P<code>[A-Za-zА-Яа-я0-9\-\(\)\.,_\*\/]+)\s+"       # 2. Позначення (БЕЗ ПРОБІЛІВ)
    ...
    for m in pattern.finditer(pdf_text):
        ...
        designation = _safe_text(m.group("code"))
        qty = _to_decimal(m.group("qty"))
        length = _to_decimal(m.group("len"))
        width = _to_decimal(m.group("wid"))
        
        # --- ФІЛЬТРАЦІЯ ВІД СМІТТЯ ---
        # 1. Якщо це випадкові цифри, і кількість/розміри дорівнюють 0 - пропускаємо
        if qty == 0 or length == 0 or width == 0:
            continue
            
        # 2. Справжні позначення (напр. 12(26).01.01.003) досить довгі. 
        # Якщо код коротший за 4 символи, це, швидше за все, випадкове сміття з PDF.
